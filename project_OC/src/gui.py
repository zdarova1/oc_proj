...
#socket_.connect((host, port))
       ...
       # self.navigation_frame.grid_rowconfigure(2, weight=1)
        ...
        #buttons        
        self.add_button = ctk.CTkButton(self.navigation_frame, text=' upload file', image=add_image, compound='left', height=40, fg_color='#C57A44')
        self.add_button.grid(row=1, column = 0, sticky = 'ew', padx=10, pady=5)
    ...
    def encode_main(self):
        ...
        self.button_file = ctk.CTkButton(self.main_frame, corner_radius=0, text = 'Private key', fg_color='#C57A44', command=lambda: self.pick_file(('Pem Files', '*.pem'), 'private_key'))
        self.privateKey_file = ctk.CTkButton(self.main_frame, corner_radius=0, text = 'Signing file', fg_color='#C57A44', command=lambda: self.pick_file(('All files', '*.*'), 'encode_file_path'))
        ...
        self.sign_file = ctk.CTkButton(self.main_frame, corner_radius=0, text = 'Make sign', fg_color='#C57A44', 
                                       command=lambda : ServerMeet(self.client, f"send_encode_file {self.encode_file_path}"))
        
        
        self.button_file.grid(row=1, column = 1, sticky = 'ew', padx=10, pady=5)
        self.privateKey_file.grid(row=2, column = 1, sticky = 'ew', padx=10, pady=5)
        self.sign_file.grid(row=3, column=1, sticky = 'ew', padx=10, pady=5)
   ...
   # def pick_and_send(self, name):
    ...
    def verify_main(self):
        ...
        self.publicKey_file = ctk.CTkButton(self.main_frame, corner_radius=0, text = 'Public key', fg_color='#C57A44', command=lambda: self.pick_file(('Pem Files', '*.pem'), 'public_key'))
        self.signed_file = ctk.CTkButton(self.main_frame, corner_radius=0, text = 'Signed file', fg_color='#C57A44', command=lambda: self.pick_file(('All files', '*.*'), 'signed_file'))
        self.source_file = ctk.CTkButton(self.main_frame, corner_radius=0, text = 'Source file', fg_color='#C57A44', command=lambda: self.pick_file(('All files', '*.*'), 'source_file'))
        self.verify_file = ctk.CTkButton(self.main_frame, corner_radius=0, text = 'Check sign', fg_color='#C57A44', command=lambda : 
                            self.VerifyFile_(path_sig = self.verify_file_path, path_source=self.source_file, public_key=self.public_key))
    ...
    def VerifyFile_(self, path_sig, path_source, public_key):
        if VerifyFile(path_sig=path_sig, path_source=path_source, public_key=public_key) == True:
