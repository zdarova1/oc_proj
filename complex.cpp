...
{
    int size = min_(z1.size(), z2.size()) - 2, i = 0; //, i = 0;
    __m256d z1_4word_, z2_4word_, res;

    //    double d[4];

#pragma omp parallel private(i, z1_4word_, z2_4word_, res) shared(size, z1, z2)
    {
#pragma omp for
        for (i = 0; i <= size; i += 2)
        {
...
{
    // z1*z2 = (a.re*b.re-a.im*b.im)+(a.re*b.im + a.im*b.re)i
    int size = min_(z1.size(), z2.size());
    Complex a, b;
    for (int i = 0; i < size; ++i)
    {
...
void complexVecMulSimd(std::vector<Complex> &z1, std::vector<Complex> &z2)
{
    ...
    {                                       //   3   2   1    0
        ...
        z2_perm = _mm256_permute4x64_pd(z2_, 0b10110001); //[im[3], im[3], im[1], im[1]]

        z1_perm = _mm256_permute4x64_pd(z1_, 0b11110101); // -[re2, im3, re0, im1]-
...
void complexVecMulSimdPar(std::vector<Complex> &z1, std::vector<Complex> &z2, int Threads)
{
    ...
    //
    Complex a, b;
    int i, size = min_(z1.size(), z2.size()) - 2;
    
#pragma omp parallel shared(z1, z2)
    {
        __m256d z1_, z2_, res, mul_res1, mul_res2, z1_perm, z2_perm, mask, mul_masked, res_;
        mask = _mm256_set_pd(1, -1, 1, -1);
        #pragma omp for 
        for (i = 0; i <= size; i += 2)          // a*b= are*bre - aim*bim + (are*bim + aim*bre)i
        {                                       //   3   2   1    0
            z1_ = _mm256_loadu_pd(&(z1[i].re)); // [ im3, re2, im1, re0]
            z2_ = _mm256_loadu_pd(&(z2[i].re)); // [ im3, re2, im1, re0]
...
void complexVecMulPar(std::vector<Complex> &z1, std::vector<Complex> &z2, int Threads)
{
    ...
    int i, size = min_(z1.size(), z2.size());
    Complex a, b;
#pragma omp parallel private(i, a, b) shared(z1, z2)
    {
        #pragma omp for
        for (i = 0; i < size; ++i)
        {
